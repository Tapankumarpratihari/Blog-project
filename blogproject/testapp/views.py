...
def mail_sendview(request,id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject,message,'tapan@blog.com',[cd['to']])
            sent=True
    else:
        form=EmailSendForm()
    return render(request,'testapp/sharebymail.html',{'post':post,'sent':sent,'form':form})